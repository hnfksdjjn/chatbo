...
# criar funcao que retorna noticias dos campeonatos estaduais
...
def verificar_palavras(texto, palavras_chave, correcoes):
    ...
    if not palavras_incorretas:
        ...
        sugestoes_correcao = {palavra: correcoes.get(palavra, "Nenhuma sugestão disponível") for palavra in palavras_incorretas}
        return f"Palavras incorretas: {', '.join(palavras_incorretas)}\nSugestões de correção: {sugestoes_correcao}"


def start():
    return 'Olá! Eu sou a analista esportiva cleo, o que voce quer saber?'
...
def handle_text(user_input, correcoes):
    ...
    # Adiciona uma resposta sobre acessar um site
        ...
        falar_resultados()
        return "Falando os resultados dos jogos em tempo real."
